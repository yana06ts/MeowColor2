...
import { PuzzleTemplate, GACHA_EXCLUSIVE_PUZZLES, LEVEL_SEQUENCE } from "../data/puzzles";
...
export function CatRoom({
  ...
  // Custom Decoration Shop & Placement states
  const [isDesignOpen, setIsDesignOpen] = useState<boolean>(false);
  const [designSection, setDesignSection] = useState<"furniture" | "rugs" | "wallpaper">("furniture");

  const [purchasedRugs, setPurchasedRugs] = useState<string[]>(() => {
    const saved = localStorage.getItem("meowcolor_purchased_rugs");
    if (saved) {
  ...
  const [purchasedWallpapers, setPurchasedWallpapers] = useState<string[]>(() => {
    const saved = localStorage.getItem("meowcolor_purchased_wallpapers");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return ["stripes"];
  ...
  const SHOP_ITEMS = [
    { id: "cushion", name: "Королевский диван 🛋️", price: 220, description: "Просторный мягкий диван для всей кошачьей банды. +25% к сбору пряжи! ⚡" },
    { id: "golden_fish", name: "Миска с карасями 🥣", price: 150, description: "Полная миска свежих карасей для сытого мурчания. +15% к сбору пряжи! ⚡" },
    { id: "tunnel", name: "Коробка мечты 📦", price: 280, description: "Простая коробка — идеальный замок котика. +15% к сбору пряжи! ⚡" },
    { id: "luxury_tower", name: "Кото-Небоскрёб 🏰", price: 650, description: "Башня-лежанка с мягкими гамаками. +45% к сбору пряжи! ⚡" },
    { id: "cactus_scratch", name: "Кактус-когтеточка 🌵", crystalPrice: 35, description: "Стильная когтеточка в форме зеленого кактуса. +50% к сбору пряжи! ⚡" },
    { id: "aquarium", name: "Аквариум 🐠", crystalPrice: 20, description: "Подводный мир с рыбками прямо в комнате! +30% к сбору пряжи! ⚡" },
  ];
  ...
  const handleBuyShopItem = (item: typeof SHOP_ITEMS[0]) => {
    if (purchasedItems.includes(item.id)) {
      ...
      if (isCurrentlyPlaced) {
        const updated = placedCats.filter(placed => placed.shopId !== item.id);
        setPlacedCats(updated);
        localStorage.setItem("meowcolor_placed_cats", JSON.stringify(updated));
        SOUNDS.playPop(0.85);
      } else {
        const newPlaced: PlacedCat = {
          id: `placed_shop_${item.id}_${Date.now()}`,
          type: "shop",
          shopId: item.id,
          name: item.name.replace(/[🧸🐚🛋️🌳🥣🌀🐱🐾🌸🪵🌌📦🏰🐠🌵]/g, "").trim(),
          x: 25 + Math.random() * 50,
          y: 45 + Math.random() * 20,
          isSleeping: false,
          flipped: Math.random() > 0.5,
        };
        const updated = [...placedCats, newPlaced];
        setPlacedCats(updated);
        localStorage.setItem("meowcolor_placed_cats", JSON.stringify(updated));
        SOUNDS.playPop(1.15);
      }
    ...
    if (item.crystalPrice !== undefined) {
      if (goldYarnCount < item.crystalPrice) {
        SOUNDS.playError();
        alert("Недостаточно кристаллов! 💎");
        return;
      }
      updateGoldYarn(goldYarnCount - item.crystalPrice);
    } else if (item.price !== undefined) {
      if (yarnCount < item.price) {
        SOUNDS.playError();
        alert("Недостаточно мотков пряжи! 🧶");
        return;
      }
      updateYarn(yarnCount - item.price);
    }
  ...
  const handleSelectRug = (rug: typeof RUG_THEMES[0]) => {
    ...
    if (rug.crystalPrice !== undefined) {
      if (goldYarnCount < rug.crystalPrice) {
        SOUNDS.playError();
        alert("Недостаточно кристаллов! 💎");
        return;
      }
      updateGoldYarn(goldYarnCount - rug.crystalPrice);
    } else if (rug.price !== undefined) {
      if (yarnCount < rug.price) {
        SOUNDS.playError();
        alert("Недостаточно мотков пряжи! 🧶");
        return;
      }
      updateYarn(yarnCount - rug.price);
    }

    const nextPurchased = [...purchasedRugs, rug.id];
  ...
  const handleSelectWallpaper = (wall: typeof WALLPAPER_THEMES[0]) => {
    ...
    if (wall.crystalPrice !== undefined) {
      if (goldYarnCount < wall.crystalPrice) {
        SOUNDS.playError();
        alert("Недостаточно кристаллов! 💎");
        return;
      }
      updateGoldYarn(goldYarnCount - wall.crystalPrice);
    } else if (wall.price !== undefined) {
      if (yarnCount < wall.price) {
        SOUNDS.playError();
        alert("Недостаточно мотков пряжи! 🧶");
        return;
      }
      updateYarn(yarnCount - wall.price);
    }

    const nextPurchased = [...purchasedWallpapers, wall.id];
    ...
    <div className="flex flex-col h-full bg-[#fceee3]">
      ...
      <div className="flex justify-between items-center px-4 py-3 bg-white/95 border-b border-rose-100 shadow-xs z-10">
        ...
        <div className="flex gap-1">
          ...
          <button
            id="design-control-btn"
            onClick={() => {
              setIsDesignOpen(true);
              SOUNDS.playPop(1.3);
            }}
            className="p-1.5 rounded-lg border text-xs font-pixel flex items-center justify-center gap-1 cursor-pointer transition-colors bg-amber-100 border-amber-200 text-amber-800 hover:bg-amber-200"
            title="Обустроить и украсить комнату"
          >
            <span className="text-sm">🛋️</span>
            <span className="text-[9px] uppercase font-bold scale-85">Дизайн</span>
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        ...
        {/* DESIGN & DECORATION CUSTOM DRAWER OVERLAY */}
        {isDesignOpen && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-fade-in">
            <div className="absolute inset-0" onClick={() => setIsDesignOpen(false)} />
            
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-white rounded-3xl p-4.5 shadow-2xl border-2 border-amber-100 max-w-sm w-full max-h-[85%] flex flex-col relative z-50 text-slate-800"
            >
              <button
                onClick={() => setIsDesignOpen(false)}
                className="absolute top-3.5 right-3.5 w-7 h-7 bg-slate-100 text-slate-500 font-bold rounded-full flex items-center justify-center hover:bg-slate-200 cursor-pointer text-xs"
              >
                ✕
              </button>

              <div className="text-center pb-2 border-b border-amber-100 shrink-0">
                <h3 className="text-xs font-pixel font-black text-amber-700 uppercase flex items-center justify-center gap-1.5 pt-1">
                  🛋️ Уютный Дизайн 🎨
                </h3>
                {/* Balance counters */}
                <div className="flex gap-3 justify-center items-center mt-1.5 text-[10px] font-pixel">
                  <span className="flex items-center gap-0.5 text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100 font-bold">
                    🧶 {yarnCount}
                  </span>
                  <span className="flex items-center gap-0.5 text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 font-bold">
                    💎 {goldYarnCount}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-1 bg-amber-50/50 p-1 rounded-xl my-2.5 border border-amber-100 shrink-0">
                {(["furniture", "rugs", "wallpaper"] as const).map((sec) => (
                  <button
                    key={sec}
                    onClick={() => {
                      setDesignSection(sec);
                      SOUNDS.playPop(1.1);
                    }}
                    className={`flex-1 text-[9px] font-pixel py-1.5 rounded-lg transition-all font-bold cursor-pointer ${
                      designSection === sec
                        ? "bg-amber-500 text-white shadow-xs"
                        : "text-amber-800 hover:bg-amber-100/50"
                    }`}
                  >
                    {sec === "furniture" ? "🛋️ Мебель" : sec === "rugs" ? "🧶 Ковры" : "🪵 Обои"}
                  </button>
                ))}
              </div>

              {/* Scrollable grid of customizable decors */}
              <div className="flex-1 overflow-y-auto pr-0.5 space-y-2.5 max-h-[300px] scrollbar-thin select-none">
              ...
              <div className="pt-2 border-t border-amber-100 shrink-0 mt-1.5">
                <button
                  onClick={() => setIsDesignOpen(false)}
                  className="w-full bg-amber-600 hover:bg-amber-700 text-white font-extrabold py-1.5 rounded-xl text-[8.5px] font-pixel transition-colors cursor-pointer uppercase tracking-wide"
                >
                  Закрыть Дизайн 🛋️✨
                </button>
              </div>
