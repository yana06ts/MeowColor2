...
const startBg = new URL(
  "./assets/images/meow_start_bg_1782757170611.jpg",
  import.meta.url,
).href;
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<
    "achievements" | "shop" | "levels" | "gacha" | "room"
  >("levels");
  const [selectedPuzzle, setSelectedPuzzle] = useState<PuzzleTemplate | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const savedClaimedAchievements = localStorage.getItem(
      "meowcolor_claimed_achievements",
    );
    ...
    const savedGachaPuzzles = localStorage.getItem(
  ...
  useEffect(() => {
    if (gameStarted && activeTab === "room") {
      const isHouseTutorialDone =
        localStorage.getItem("meowcolor_tutorial_house") === "completed";
  ...
  const handlePixelColored = (index: number | number[]) => {
    ...
    localStorage.setItem(
      `meowcolor_progress_${selectedPuzzle!.id}`,
    ...
    const filledWithColor = updated.filter(
      (c) => c.number === targetNum && c.filled,
    ).length;
    const wasAlreadyDone =
      currentProgress.filter((c) => c.number === targetNum && c.filled)
        .length === totalWithColor;

    if (
      filledWithColor === totalWithColor &&
      !wasAlreadyDone &&
      totalWithColor > 0
    ) {
      ...
      const nextColor = selectedPuzzle!.colors.find((col) => {
        ...
        const filled = updated.filter(
          (c) => c.number === col.number && c.filled,
        ).length;
        return filled < total;
      });
      if (nextColor) {
            ...
            rowStr.split("").map((c, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    backgroundColor:
                      num === 0 ? "transparent" : color?.hex || "#999",
                  }}
                />
              );
            }),
  ...
  const handleResetApplicationData = () => {
    if (
      confirm(
        "Вы уверены, что хотите сбросить прогресс? Это очистит ваших котиков и раскраски.",
      )
    ) {
      localStorage.clear();
      setCompletedPuzzles([]);
          ...
          >
            ...
            {showExitConfirm && (
              <div className="absolute inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center animate-fade-in">
                <div className="bg-slate-900/90 border border-rose-300/20 rounded-3xl p-6 max-w-[280px] shadow-2xl">
                  <span className="text-4xl block mb-3 animate-bounce">
                    🐾😿
                  </span>
                  ...
                  <p className="text-[10px] text-slate-300 mt-2 leading-relaxed font-semibold">
                    Котики будут очень скучать без твоего внимания и ярких
                    красок! Останься ещё ненадолго. ❤️
                  </p>
            ...
            {showExitScreen && (
              <div
                ...
                style={{
                  backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.95)), url(${startBg})`,
                }}
              >
                <div className="max-w-[280px] flex flex-col items-center">
                  ...
                  <p className="text-[10px] text-rose-100/70 mt-3 leading-relaxed font-semibold">
                    Котики уснули сладким сном в ожидании твоего возвращения.
                    Твой прогресс надёжно сохранён на этом устройстве!
                  </p>
              ...
              <div className="flex justify-between items-center">
                ...
                >
                  ...
                  <div className="flex flex-col leading-none">
                    <span className="text-xs font-pixel tracking-wider scale-90 origin-left">
                      MEOWCOLOR
                    </span>
                  </div>
                </div>
                    ...
                    {colorCompletedSplash !== null && (
                      <div className="absolute inset-x-0 top-6 z-50 flex justify-center animate-bounce pointer-events-none px-4 text-center">
                        <div className="bg-emerald-500 text-white font-pixel text-[9px] px-3.5 py-1.5 rounded-full shadow-lg border border-emerald-300 flex items-center gap-1 uppercase font-bold">
                          <span>✨</span>
                          <span>
                            ЦВЕТ {colorCompletedSplash} ПОЛНОСТЬЮ ЗАВЕРШЕН! 🐾
                          </span>
                          <span>✨</span>
                        </div>
                      </div>
                          ...
                          <div className="flex justify-center mt-1">
                            {tutorialStep === 3 ? (
                              <span className="text-[8px] font-pixel text-amber-600 font-bold tracking-tight animate-pulse flex items-center gap-1 py-1">
                                <span>✨</span>{" "}
                                <span>Волшебство начинается...</span>{" "}
                                <span>✨</span>
                              </span>
                            ) : (
                              <span
                  ...
                  <div className="bg-white border-t border-rose-100 pt-2 pb-5 px-3 flex flex-col gap-1.5 shrink-0 select-none">
                    ...
                    <div className="flex gap-3 overflow-x-auto py-2.5 px-3 justify-start sm:justify-center no-scrollbar">
                      {selectedPuzzle.colors.map((color) => {
                        ...
                        const filledCount = currentProgress.filter(
                          (c) => c.number === color.number && c.filled,
                        ).length;
                        ...
                        return (
                          ...
                          >
                            ...
                            {!isDone ? (
                              ...
                              <span className="text-white drop-shadow-md text-xs font-bold">
                                ✓
                              </span>
                            )}
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                  ...
                  {activeTab === "levels" && (
                    <div className="flex-1 flex flex-col overflow-hidden bg-[#FFFBF0] animate-fade-in relative">
                      ...
                      {(() => {
                        const currentLvl =
                          LEVEL_SEQUENCE[currentLevelIndex] ||
                          LEVEL_SEQUENCE[0];
                          ...
                          <div className="mx-4 mt-4 bg-gradient-to-r from-amber-100 via-orange-100 to-amber-50 border-2 border-amber-200 rounded-3xl p-3.5 flex flex-col gap-2 shrink-0 select-none shadow-sm">
                            <div className="flex justify-between items-center text-[9.5px] font-pixel text-amber-900 font-black uppercase">
                              <span className="flex items-center gap-1">
                                👑 Прогресс Главы:{" "}
                                <span className="text-rose-600">{catName}</span>
                              </span>
                              <span className="bg-amber-200 text-amber-950 px-2 py-0.5 rounded-full border border-amber-300 font-bold">
                      ...
                      >
                        {(() => {
                          ...
                          const currentPuzzle = allAvailablePuzzles.find(
                            (p) => p.id === currentLvl.puzzleId,
                          );
                          ...
                          const diffLabel =
                            diffLabels[currentPuzzle.difficulty] || "НОРМА";
                      ...
                      {houseTutorialStep !== null && (
                        <div className="absolute inset-x-0 top-14 z-50 flex flex-col items-center px-4 pointer-events-none">
                          <div className="bg-[#FFF6E5] text-[#5C3A21] p-3.5 rounded-2xl border-2 border-amber-300 shadow-2xl max-w-xs text-center animate-fade-in flex flex-col gap-1.5 select-none relative pointer-events-auto">
                            ...
                            <p className="text-[10px] leading-relaxed font-semibold mt-1">
                              {houseTutorialStep === 1 &&
                                "Смотри! Твоя комната пуста, но у тебя есть свободный котик! 🐱 Нажми на котика со значком «Призвать» на нижней полке, чтобы впустить его в домик!"}
                              {houseTutorialStep === 2 &&
                                "Отлично! Твой котик появился в комнате! Теперь зажми его и перетащи на мягкий круглый коврик в центре! 🧶🐾"}
                              {houseTutorialStep === 3 &&
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center bg-[#FFFBF0] animate-fade-in p-4 space-y-4 overflow-y-auto">
                      ...
                      <div className="bg-white border-2 border-rose-100 rounded-[32px] p-6 shadow-md flex flex-col items-center w-full max-w-xs shrink-0 select-none">
                        ...
                        <div
                          ...
                          onClick={() => {
                            ...
                            if (!costTickets && !costYarn) {
                              SOUNDS.playError();
                              alert(
                                "Не хватает Купонов (нужно 1 🎟️) или Пряжи (нужно 100 🧶)!",
                              );
                              return;
                            }
                        ...
                        >
                          ...
                          <span className="absolute top-2 left-2 text-xl animate-pulse">
                            ✨
                          </span>
                          <span className="absolute top-0 right-4 text-2xl animate-bounce">
                            🌟
                          </span>
                          <span className="absolute bottom-10 -left-4 text-lg animate-pulse">
                            ✨
                          </span>
                          <span className="absolute bottom-6 -right-3 text-xl animate-bounce">
                            📦
                          </span>
                          ...
                          <div className="relative w-44 h-36 flex flex-col items-center justify-end">
                            ...
                            <div className="absolute top-1 z-10 text-3xl animate-bounce flex items-center justify-center">
                              🐱
                            </div>
                        ...
                        <div className="w-full mt-2 flex gap-2 shrink-0">
                          <button
                            ...
                            onClick={() => {
                              const clicker = document.getElementById(
                                "gacha-gift-box-clicker",
                              );
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden bg-[#FFFBF0] animate-fade-in p-4 space-y-4">
                      ...
                      <div className="flex-1 overflow-y-auto space-y-3 scrollbar-thin select-none">
                        {[
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleBuyPowerup(booster.type, booster.price)
                                }
                        ...
                        <div className="flex items-center gap-1.5 pb-1">
                          <span className="text-rose-500 text-xs animate-pulse">
                            💎
                          </span>
                        ...
                        <div className="space-y-3 pb-4">
                          ...
                          <div className="bg-gradient-to-r from-amber-50/50 to-orange-50/30 border border-amber-200/60 rounded-2xl p-3 flex gap-3 items-center justify-between shadow-3xs relative overflow-hidden">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[8.5px] text-slate-500 mt-1 font-semibold leading-relaxed">
                                ...
                                <span className="text-emerald-600">2 🎟️</span> +{" "}
                                <span className="text-cyan-600">5 💎</span>
                              </p>
                              <span className="text-[7px] text-slate-400 font-bold block mt-0.5 uppercase tracking-wide">
                                Внутри: милые угощения!
                              </span>
                            </div>
                          ...
                          <div className="bg-gradient-to-r from-rose-50/40 to-pink-50/30 border border-pink-200/60 rounded-2xl p-3 flex gap-3 items-center justify-between shadow-3xs relative overflow-hidden">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[8.5px] text-slate-500 mt-1 font-semibold leading-relaxed">
                                Содержимое:{" "}
                                <span className="text-amber-600">2000 🧶</span>{" "}
                                +{" "}
                                <span className="text-emerald-600">10 🎟️</span>{" "}
                                + <span className="text-cyan-600">25 💎</span>
                              </p>
                              <span className="text-[7px] text-slate-400 font-bold block mt-0.5 uppercase tracking-wide">
                                Отличный пушистый буст!
                              </span>
                            </div>
                          ...
                          <div className="bg-gradient-to-r from-indigo-50/20 to-violet-50/20 border border-violet-200/60 rounded-2xl p-3 flex gap-3 items-center justify-between shadow-3xs relative overflow-hidden">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[8.5px] text-slate-500 mt-1 font-semibold leading-relaxed">
                                Содержимое:{" "}
                                <span className="text-amber-600">10000 🧶</span>{" "}
                                +{" "}
                                <span className="text-emerald-600">50 🎟️</span>{" "}
                                + <span className="text-cyan-600">100 💎</span>
                              </p>
                              <span className="text-[7px] text-slate-400 font-bold block mt-0.5 uppercase tracking-wide">
                                Достойно Кошачьего Лорда!
                              </span>
                            </div>
                ...
                <div className="absolute inset-x-0 top-0 bottom-14 bg-slate-900/40 z-25 pointer-events-none flex flex-col justify-end">
                  <div className="bg-[#FFF6E5] border-2 border-amber-300 text-amber-950 p-3.5 rounded-2xl shadow-2xl mx-4 mb-3 pointer-events-auto text-center flex flex-col gap-1.5 select-none relative animate-fade-in">
                    ...
                    <p className="text-[10px] font-semibold text-slate-800 leading-relaxed">
                      ...
                      в твоей новой уютной комнате. Нажми на вкладку «Домик»
                      внизу! 👇
                    </p>
                ...
                <div className="bg-white rounded-3xl p-5 shadow-2xl border-2 border-rose-100 max-w-sm w-full relative select-none">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-rose-100/20">
                      <div className="flex-1">
                        <span className="text-xs font-pixel text-indigo-700">
                          🪄 Палочка
                        </span>
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-rose-100/20">
                      <div className="flex-1">
                        <span className="text-xs font-pixel text-amber-700">
                          💣 Бомбочка
                        </span>
                        <p className="text-[10px] text-slate-500 leading-tight">
